...
Game::~Game()
{
	...
	ppRTV->Release();		// Allows us to render to a texture
	ppSRV->Release();	// Allows us to sample from the same texture
	ppbRTV->Release();		// Allows us to render to a texture
	ppbSRV->Release();	// Allows us to sample from the same texture
	ppbloomRTV->Release();		// Allows us to render to a texture
	ppbloomSRV->Release();
...
void Game::Init()
{
	...
	CreateWICTextureFromFile(device, context, L"Assets/Textures/01.jpg",0, &SRV_Metal);
	...
	CreateDDSTextureFromFile(device, L"Assets/Textures//SkyBox.dds", 0, &skySRV);
	//CreateDDSTextureFromFile(device, L"Assets/Textures//0.dds", 0, &skySRV);

	CreateWICTextureFromFile(device, context, L"Assets/Textures/volcanoes-e1269738162549.jpg", 0, &srvlight);
	...
	emitter_fog = new Emitter(
		...
		35.0,							// Particle lifetime
		65.0f,							// Start size
		65.0f,							// End size
		XMFLOAT4(0.5, 0.3, 1.1, 0.4),		// Start color
		XMFLOAT4(0.5, 0.3, 1.2, 0.4),		// End color
		XMFLOAT3(-10, 20, 0),			// Start velocity
		XMFLOAT3(-20, -20, 50),			// Start position
		XMFLOAT3(50, -5, 0),			// Start acceleration
		device,
		particleVS,
		particlePS,
	...
	emitter_fog1 = new Emitter(
		1,								// Max particles
		1,								// Particles per second
		35.0,							// Particle lifetime
		30.0f,							// Start size
		30.0f,							// End size
		XMFLOAT4(0.1, 0.1, 0.1, 0.5),		// Start color
		XMFLOAT4(0.1, 0.1, 0.1, 0.5),		// End color
		XMFLOAT3(-10, 20, 0),			// Start velocity
		XMFLOAT3(0, -20, 50),			// Start position
		XMFLOAT3(50, -5, 0),			// Start acceleration
		device,
		particleVS,
		particlePS,
	...
	XMFLOAT3(-10, 20, 0),			// Start velocity
	XMFLOAT3(20, 20, 0),			// Start position
	XMFLOAT3(50, -5, 0),			// Start acceleration
	device,
	particleVS,
	particlePS,
	...
	*/
	...
	scoreText[3] = new UI(device, context, L"Your target: ", XMFLOAT4(+200.0f, +0.0f, +110.0f, 110.0f));
	...
	// check the slient mode via IsAudioDevicePresent
	...
	// adding sounds
	m_explode = std::make_unique<SoundEffect>(m_audEngine.get(), L"Assets/Sounds/Explo2.wav");
	m_ambient = std::make_unique<SoundEffect>(m_audEngine.get(),
		L"Assets/Sounds/JungleBackground.wav");

	// for playing a sound one time (explosion)
...
void Game::LoadShaders()
{
	...
	boVS = new SimpleVertexShader(device, context);
	if (!boVS->LoadShaderFile(L"Debug/BloomObjectVS.cso"))
		boVS->LoadShaderFile(L"BloomObjectVS.cso");

	boPS = new SimplePixelShader(device, context);
	if (!boPS->LoadShaderFile(L"Debug/BloomObjectPS.cso"))
		boPS->LoadShaderFile(L"BloomObjectPS.cso");
...
void Game::Update(float deltaTime, float totalTime)
{
	...
	emitter_fog1->Update(deltaTime);
	emitter_fog2->Update(deltaTime);
	...
	LightSource->SetScale(XMFLOAT3(0.5, 0.5, 0.5));

	LightSource->SetTrans(XMFLOAT3(6.3, 3.1, 0));
	// audio update
	if (m_retryAudio)
	{
		m_retryAudio = false;
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	if (level == 1) {
		...
		boVS->SetMatrix4x4("world", LightSource->GetMatrix());
		boVS->SetMatrix4x4("view", c->GetViewMatrix());
		boVS->SetMatrix4x4("projection", c->GetProjectionMatrix());

	
		boVS->CopyAllBufferData();
		boVS->SetShader();
		//VS_Shadow->deviceContext->PSSetShader(0, 0, 0);

		boPS->SetData("dl",&directionalLight,sizeof(DirectionalLight));
		boPS->SetData("dl0",&directionalLight2,sizeof(DirectionalLight));
		boPS->SetData("pl",&pointLight,sizeof(PointLight));

		boPS->SetSamplerState("basicSampler", SampleState);
		
		boPS->SetShaderResourceView("diffuseTexture", ppbloomSRV);


		//pixelShader->SetData("pl", &pl, sizeof(PointLight));

		boPS->CopyAllBufferData();
		boPS->SetShader();
		...
		emitter_fog1->Draw(context, c);
		emitter_fog2->Draw(context, c);
		// Particle states end
